...
def has_sin?(string)
  /\b\d\d\d[.-]?\d\d\d[.-]?\d\d\d\b/.match(string) ? true : false
end
...
def grab_sin(string)
  matched = (string).scan(/\b\d\d\d-\d\d\d-\d\d\d\b/)
  matched.any? ? matched[0] : nil
end
...
def hide_all_sins(string)
  hidden_sin = string.gsub(/\b\d\d\d-\d\d\d-\d\d\d\b/) { |string| "XXX-XXX-" + string[8..-1] }
  hidden_sin ? hidden_sin : string
end
...
def format_sins(string)
  subbed = (string).gsub!(/\b\d\d\d[.-]?\d\d\d[.-]?\d\d\d\b/) do |sin_array|
    sin_array = sin_array.split(/[.-]?/)
    "%s%s%s-%s%s%s-%s%s%s" % sin_array
  end

  subbed ? subbed : string
end
